...
def main(page: Page):
    ...
    def color_change(e):
        ...
        global color
        color = choice(_color)
        banner.controls = [Text(spans=[TextSpan('XM-SMS OTP', TextStyle(size=85, foreground=Paint(color=color, stroke_width=9, stroke_join='round', style='stroke')))], font_family='Consolas'), Text(spans=[TextSpan('XM-SMS OTP', TextStyle(size=95, color=color))], font_family='Consolas')]
    ...
    banner = Stack([Text(spans=[TextSpan('XM-SMS OTP', TextStyle(size=85, foreground=Paint(color=color, stroke_width=7, stroke_join='round', style='stroke')))], font_family='Consolas'), Text(spans=[TextSpan('XM-SMS OTP', TextStyle(size=85, color=color))], font_family='Consolas')])

    number = TextField(label='Telefon raqamni kriting "+"', width=275, text_align='center', border_radius=40, border_color=color, cursor_color=color, focused_border_color=color, autofocus=True, selection_color=color, label_style=TextStyle(color=color))

    replay = TextField(label='Soni', width=131, text_align='center', border_radius=40, border_color=color, cursor_color=color, focused_border_color=color, selection_color=color, value='1', label_style=TextStyle(color=color))

    type_attack = Dropdown(label='Hujum turi', hint_text='choose Attack type', options=[dropdown.Option('MIX'), dropdown.Option('SMS'), dropdown.Option('CALL')], width=131, border_radius=40, alignment=alignment.bottom_center, border_color=color, value=check_config()['type_attack'], label_style=TextStyle(color=color), on_change=type_attack_change)

    feedback = Switch(label='Feedback servislar (?)', value=True if check_config()['feedback'] == 'True' else False, width=100, active_color=color, on_change=feedback_change, tooltip='Services that leave applications (such as connecting to the Internet or taking out a loan) on different sites. Be careful\nwhen using this function!')

    attack_button = ElevatedButton(content=Text('Hujumni boshlash', size=25), on_click=checking_values, width=190, height=60, color=color)
    ...
    def ADD():
        page.add(
                ...
                feedback,
                attack_button,
                  Row([IconButton(icon='telegram', icon_size=48, tooltip='Channel', url='https://t.me/pmurod', icon_color=color),
